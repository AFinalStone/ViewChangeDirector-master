package com.shi.androidstudy.loadingprogressbar;
...
import com.shi.androidstudy.loadingprogressbar.indicator.BallPulseIndicator;
...
//                attrs, R.styleable.AVLoadingIndicatorView, defStyleAttr, 0);
...
//        mIndicatorColor = a.getColor(R.styleable.AVLoadingIndicatorView_indicatorColor, Color.WHITE);
         ...
         * 获得我们所定义的自定义样式属性
         */
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.AVLoadingIndicatorView, defStyleAttr, 0);
        int length = a.getIndexCount();
        for (int i = 0; i < length; i++) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.AVLoadingIndicatorView_minWidth) {// mImage = BitmapFactory.decodeResource(getResources(), a.getResourceId(attr, 0));
                mMinWidth = a.getDimensionPixelSize(attr, mMinWidth);
            } else if (attr == R.styleable.AVLoadingIndicatorView_maxWidth) {
                mMaxWidth = a.getDimensionPixelSize(attr, mMaxWidth);
            } else if (attr == R.styleable.AVLoadingIndicatorView_minHeight) {
                mMinHeight = a.getDimensionPixelSize(attr, mMinHeight);
            } else if (attr == R.styleable.AVLoadingIndicatorView_maxHeight) {
                mMaxHeight = a.getDimensionPixelSize(attr, mMaxHeight);
            } else if (attr == R.styleable.AVLoadingIndicatorView_indicatorName) {
                String indicatorName = a.getString(attr);
                if (mIndicator == null) {
                    setIndicator(DEFAULT_INDICATOR);
                }
                setIndicator(indicatorName);
            } else if( attr == R.styleable.AVLoadingIndicatorView_indicatorColor){
                mIndicatorColor = a.getColor(attr, Color.WHITE);
            }
        }
        a.recycle();
    }
